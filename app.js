// ===== Queue‑Monitor Scheduler logic – fixed PDF export ===== //
...
const formDiv = $('team-form');
const addBtn  = $('add-member');
const genBtn  = $('generate');
const copyBtn = $('copy-table');
const saveBtn = $('save-day');
const icsBtn  = $('export-ics');
const xlsxBtn = $('export-xlsx');
...
function makeRow([name,start,end] = ['', '09:00', '17:00']) {
  ...
  `;
  row.draggable = true;
  row.ondragstart = e => { window._drag=row;row.classList.add('dragging');e.dataTransfer.effectAllowed='move'; };
...
function buildSchedule() {
  ...
  const mLen=+$('morning-len').value||6;
  const aLen=+$('afternoon-len').value||15;
  const strategy=$('strategy-mode').value;
  ...
  while(cur<latest){
    ...
    let chosen;
    if(strategy==='round'){
      chosen=cands.find(p=>p===order[idx%order.length])||cands[0]; idx=(order.indexOf(chosen)+1)%order.length;
    }else{
      chosen=cands.sort((a,b)=>totals[a.name]-totals[b.name]||order.indexOf(a)-order.indexOf(b))[0];
    }
...
function render(sched){
  ...
  sched.forEach((s,i)=>{
    if(i&&s.start>sched[i-1].end){const g=document.createElement('tr');g.className='gap-row';g.innerHTML=`<td colspan=5>*** GAP ${fm(sched[i-1].end)}-${fm(s.start)} ***</td>`;tbody.appendChild(g);}    
    const tr=document.createElement('tr'); tr.innerHTML=`<td>${s.name}</td><td>${fm(s.start)}</td><td>${fm(s.end-1)}</td><td>${s.duration}</td><td>${s.next}</td>`; tbody.appendChild(tr);
  });
  ...
  const st=$('shift-times'); st.innerHTML=''; const ul=document.createElement('ul'); Object.keys(per).sort().forEach(n=>{const li=document.createElement('li');li.textContent=`${n}: ${per[n].join(', ')}`; ul.appendChild(li);}); st.appendChild(ul);
  const pipe=$('pipe-output'); if($('output-mode').value==='table'){$('schedule-table').style.display='';pipe.style.display='none';copyBtn.style.display='inline-block';}
  else{$('schedule-table').style.display='none';copyBtn.style.display='none'; pipe.style.display='block'; pipe.textContent=sched.map(s=>`${s.name}|${fm(s.start)}|${fm(s.end)}|${s.duration}|${s.next}`).join('\n');}
  [saveBtn,icsBtn,xlsxBtn,pdfBtn].forEach(b=>b.disabled=false);
}

genBtn.onclick=()=>{const s=buildSchedule();if(s)render(s);};
copyBtn.onclick=()=>navigator.clipboard.writeText([...$('schedule-table').querySelectorAll('tr')].map(r=>[...r.children].map(c=>c.textContent).join('\t')).join('\n'));

/************ Save & Load JSON (download / upload) *************/
saveBtn.onclick=()=>{
...
loadInp.onchange=e=>{const file=e.target.files[0]; if(!file)return; const reader=new FileReader(); reader.onload=ev=>{try{render(JSON.parse(ev.target.result));}catch{alert('Bad JSON');}}; reader.readAsText(file);} ;

/************ Exports *************/
icsBtn.onclick=()=>{const cal=new ics(); current.forEach((s,i)=>{const d=new Date(),e=new Date(); d.setHours(Math.floor(s.start/60),s.start%60); e.setHours(Math.floor(s.end/60),s.end%60); cal.addEvent(`${s.name} queue`,`Shift ${i+
