...
/* default roster */
const DEFAULTS=[['Anish','09:00','18:00'],['Ashish','07:30','16:30'],['Chetan','09:00','18:00'],
...
const formDiv=$('team-form'), addBtn=$('add-member'), genBtn=$('generate'),
 copyBtn=$('copy-table'), saveBtn=$('save-day'), loadInp=$('load-day'),
 icsBtn=$('export-ics'), xlsxBtn=$('export-xlsx'), pdfBtn=$('export-pdf');
...
/* Scheduler core (unchanged algorithm) */
let rotateIdx=0,current=[];
function buildSchedule(){
  const roster=[...document.querySelectorAll('.member-row')].filter(r=>r.querySelector('.avail').checked)
  ...
  const mLen=+$('morning-len').value||6, aLen=+$('afternoon-len').value||15, strat=$('strategy-mode').value;
  ...
  while(cur<latest){
    const slot=cur<720?mLen:aLen, cands=order.filter(p=>p.start<=cur&&p.end>=cur+slot);
...
function render(sched){
  ...
  sched.forEach((s,i)=>{if(i&&s.start>sched[i-1].end){const g=document.createElement('tr');g.className='gap-row';
  ...
  if($('output-mode').value==='table'){$('schedule-table').style.display='';pipe.style.display='none';copyBtn.style.display='inline-block';}
...
loadInp.onchange=e=>{
  ...
  const reader=new FileReader();reader.onload=ev=>{try{const data=JSON.parse(ev.target.result);render(data);}catch{alert('Invalid JSON');}};reader.readAsText(file);
};

/* iCal */
icsBtn.onclick=()=>{
  const cal=new ics();current.forEach((s,i)=>{const d=new Date(),e=new Date();
    d.setHours(Math.floor(s.start/60),s.start%60,0,0);e.setHours(Math.floor(s.end/60),s.end%60,0,0);
    cal.addEvent(`${s.name} queue`,`Shift ${i+1}`,'',d,e);});cal.download(`queue-${todayKey()}`);
};
...
/* PDF (table via autoTable) */
pdfBtn.onclick=()=>{
  const {jsPDF}=window.jspdf,doc=new jsPDF();doc.text(`Queue Schedule ${todayKey()}`,14,14);
  doc.autoTable({startY:20,head:[['Person','Start','End','Min']],body:current.map(s=>[s.name,fm(s.start),fm(s.end),s.duration])});
